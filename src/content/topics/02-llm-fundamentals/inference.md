...
As an application developer, inference *is* your production workload — its speed, cost, and reliability are what your users experience. Two numbers matter most: **time to first token** (how long before output starts, dominated by processing your prompt) and tokens per second (how fast it streams after that). Inference economics also explain provider pricing, why long prompts are slow and expensive, and why optimizations exist at every layer: **quantization** to shrink models, KV caching and **prompt caching** to skip recomputing shared prefixes, and batching to share GPUs across requests.
