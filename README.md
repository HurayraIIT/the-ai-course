...
- Register / login / logout / forgot + reset password, with OTP email verification: registration emails a 6-digit code (15-minute expiry, resendable, attempt-limited); submitting the code verifies the account and logs the user in. Unverified accounts cannot log in — a login attempt routes them back to the code step.
